...
log_file = '/users/ad394h/Documents/microvascular_proliferation/logs/sm_unet_effi_2_class_mvp_weights_{}.txt'.format(timestr)
...
model = torch.load('/users/ad394h/Documents/segment_blood_vessels/models/Unet_efficientnet_2_class_base_model.pt')
...
# add weights as addditional parameter for def
def fit(epochs, model, train_loader, test_loader, criterion, optimizer, scheduler,weights,patch=False):
...
weight_decay = 1e-4
# we shall add weights to the classes in favour of blood vessels, to overcome possible imbalance
weights = torch.tensor([0.4,0.8],device='cuda:0')
logger.info(f"cross entropy loss weights are {weights.cpu().numpy()}")

# we are adding weights during the actual calculation in fit, but criterion object has to be created with weights keyword
...
# passing weights to the fit function. the weights are loaded to gpu inside fit

history = fit(epochs, model, train_loader, test_loader, criterion, optimizer, sched,weights)

timestr = time.strftime("%Y%m%d-%H%M%S")
try:    
    torch.save(model, '/users/ad394h/Documents/microvascular_proliferation/model/sm_unet_eff_b7_2class_1024px_mvp_{}.pt'.format(timestr))
...
plot_loss(history)
plt.savefig("/users/ad394h/Documents/microvascular_proliferation/results/unet_eff_mvp_loss_{}.jpg".format(timestr))
plt.clf() # clear the above plot
plot_score(history)
plt.savefig("/users/ad394h/Documents/microvascular_proliferation/results/unet_eff_mvp_score_{}.jpg".format(timestr))
plt.clf()
plot_acc(history)
plt.savefig("/users/ad394h/Documents/microvascular_proliferation/results/unet_eff_mvp_accuracy_{}.jpg".format(timestr))
plt.clf()
plot_dice(history)
plt.savefig("/users/ad394h/Documents/microvascular_proliferation/results/unet_eff_mvp_dice_{}.jpg".format(timestr))
