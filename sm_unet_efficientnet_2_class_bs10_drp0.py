...
log_file = '/users/ad394h/Documents/segment_blood_vessels/logs/sm_unet_effi_2_class_bs10_drp0_{}.txt'.format(timestr)
...
#     classes = 2,
#     aux_params= {"classes":2,"dropout":0.0, "activation":None}
# )
...
def pixel_accuracy(output, mask):
    with torch.no_grad():
        output = torch.argmax(F.softmax(output, dim=1), dim=1) # output index to extract the mask 
...
def mIoU(output, mask, smooth=1e-10, n_classes=2):
    with torch.no_grad():
        pred_mask = F.softmax(output, dim=1) # output index to extract the mask 
...
for lr in max_lr_list:
    ...
    file_name = "history_bs10_drp0_" + str(lr) + ".csv"
    ...
    try:
        ...
        plot_loss(history)
        plt.savefig("/users/ad394h/Documents/segment_blood_vessels/tests/optimize_bs10_drp0_loss_{}.jpg".format(lr))
        plt.clf() # clear the above plot
        plot_score(history)
        plt.savefig("/users/ad394h/Documents/segment_blood_vessels/tests/optimize_bs10_drp0_score_{}.jpg".format(lr))
        plt.clf()
        plot_acc(history)
        plt.savefig("/users/ad394h/Documents/segment_blood_vessels/tests/optimize_bs10_drp0_accuracy_{}.jpg".format(lr))
